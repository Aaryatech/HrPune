...
import com.ats.hrpune.adapter.LeaveTrailHistoryAdapter;
...
public class LeaveDetailActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        leaveHistory = gson.fromJson(upcomingStr, LeaveHistory.class);
...
////
////                ArrayList<LeaveDetail> leaveTrailTemps = new ArrayList<>();
////
////                for (int i = 0; i < leaveHistory.getGetLeaveStatusList().size(); i++) {
////                    leaveTrailTemps.add(leaveHistory.getGetLeaveStatusList().get(i));
////                }
////
////                LeaveTrailHistoryAdapter adapter = new LeaveTrailHistoryAdapter(leaveTrailTemps, this);
...
////        }
    ...
    private void getLeaveTrail(final Integer leaveId) {
        ...
        if (Constants.isOnline(LeaveDetailActivity.this)) {
            final CommonDialog commonDialog = new CommonDialog(LeaveDetailActivity.this, "Loading", "Please Wait...");
            commonDialog.show();
                ...
                public void onResponse(Call<ArrayList<MyLeaveTrailData>> call, Response<ArrayList<MyLeaveTrailData>> response) {
                    try {
                        if (response.body() != null) {
                            ...
                            LeaveTrailAdapter adapter = new LeaveTrailAdapter(response.body(), LeaveDetailActivity.this);
                            RecyclerView.LayoutManager mLayoutManager = new LinearLayoutManager(LeaveDetailActivity.this);
                            recyclerView.setLayoutManager(mLayoutManager);
                            recyclerView.setItemAnimator(new DefaultItemAnimator());
                            recyclerView.setAdapter(adapter);
            ...
            Toast.makeText(LeaveDetailActivity.this, "No Internet Connection !", Toast.LENGTH_SHORT).show();
        }
    }
